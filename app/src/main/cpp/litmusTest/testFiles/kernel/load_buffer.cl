...
__kernel void litmus_test (
  ...
  if (shuffled_workgroup < stress_params[9]) {
    ...
    uint y_0 = id_0 * stress_params[10] * 2;
    uint x_0 = (permute_id(id_0, stress_params[8], total_ids)) * stress_params[10] * 2 + stress_params[11];
    uint x_1 = (permute_id(id_1, stress_params[8], total_ids)) * stress_params[10] * 2 + stress_params[11];
    uint y_1 = id_1 * stress_params[10] * 2;
    if (stress_params[4]) {
    ...
    uint r0 = atomic_load_explicit(&test_locations[y_0], memory_order_relaxed);
    ...
    atomic_store_explicit(&test_locations[y_1], 1, memory_order_relaxed);
