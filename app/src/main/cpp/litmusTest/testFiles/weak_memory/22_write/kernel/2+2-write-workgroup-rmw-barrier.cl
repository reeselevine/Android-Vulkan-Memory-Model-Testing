...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    atomic_store_explicit(&wg_test_locations[x_0], 2, memory_order_relaxed);
    atomic_work_item_fence(CLK_LOCAL_MEM_FENCE, memory_order_seq_cst, memory_scope_device);
    atomic_exchange_explicit(&wg_test_locations[y_0], 1, memory_order_relaxed);
    atomic_exchange_explicit(&wg_test_locations[y_1], 2, memory_order_relaxed);
    atomic_work_item_fence(CLK_LOCAL_MEM_FENCE, memory_order_seq_cst, memory_scope_device);
    atomic_store_explicit(&wg_test_locations[x_1], 1, memory_order_relaxed);
    atomic_work_item_fence(CLK_LOCAL_MEM_FENCE, memory_order_seq_cst, memory_scope_device);
    atomic_store(&test_locations[shuffled_workgroup * get_local_size(0) + stress_params[10] * 2 + x_1], atomic_load(&wg_test_locations[x_1]));
    atomic_store(&test_locations[shuffled_workgroup * get_local_size(0) + stress_params[10] * 2 + y_1], atomic_load(&wg_test_locations[y_1]));
