...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    uint r0 = atomic_load_explicit(&test_locations[y_0], memory_order_acquire);
    atomic_store_explicit(&test_locations[x_0], 1, memory_order_release);
    uint r1 = atomic_load_explicit(&test_locations[x_1], memory_order_acquire);
    atomic_store_explicit(&test_locations[y_1], 1, memory_order_release);
