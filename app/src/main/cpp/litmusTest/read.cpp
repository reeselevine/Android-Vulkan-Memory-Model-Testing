...
namespace read {
    ...
    constexpr char *TAG = "MainActivityRead";
    constexpr char *SHADER_NAME = "read.spv";
    constexpr char *OUTPUT_NAME = "read_output.txt";
    ...
    const char *testName = "read";
    const char *weakBehaviorStr = "y: 2, r0: 0";
    ...
    public:
        ...
        void checkResult(easyvk::Buffer &testData, easyvk::Buffer &results,
            ...
            if (rand() % sampleInterval == 1) {
                outputFile << "y: " << testData.load(memLocations.load(1)) << ", r0: " << results.load(0)<< "\n";
            }
            if (testData.load(memLocations.load(1)) == 2 && results.load(0) == 0) {
                weakBehavior++;
            } else {
                if (testData.load(memLocations.load(1)) == 1 && results.load(0) == 1) {
                    interBehavior++;
                }
                else {
                    seqBehavior++;
