...
namespace atomicity {
    ...
    constexpr char *TAG = "MainActivityAtomicity";
    constexpr char *SHADER_NAME = "atomicity.spv";
    constexpr char *OUTPUT_NAME = "atomicity_output.txt";
    ...
    const char* testName = "atomicity";
    const char* weakBehaviorStr = "r0: 0, x: 1";
    ...
    public:
        ...
        if (rand() % sampleInterval == 1) {
            ...
            if (results.load(0) == 0 && testData.load(memLocations.load(0)) == 1) {
                weakBehavior++;
            } else {
                nonWeakBehavior++;
            }
