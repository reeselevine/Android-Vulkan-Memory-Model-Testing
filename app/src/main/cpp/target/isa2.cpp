...
const char* testName = "isa2";
const char* weakBehaviorStr = "r0: 1, r1: 1, r2: 0";
...
public:
	...
	if (rand() % sampleInterval == 1) {
	    printf("r0: %u, r1: %u, r2: %u\n", results.load(0),results.load(1),results.load(2));
	}
        if (results.load(0) == 1 && results.load(1) == 1 && results.load(2) == 0) {
            weakBehavior++;
        } else {
            nonWeakBehavior++;
        }
