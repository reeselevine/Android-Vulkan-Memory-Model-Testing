...
const char* testName = "corr4";
const char* weakBehaviorStr = "r0: 1, r1: 2, r2: 2, r3: 1";
...
public:
	...
	if (rand() % sampleInterval == 1) {
	    printf("r0: %u, r1: %u, r2: %u, r3: %u\n", results.load(0),results.load(1),results.load(2),results.load(3));
	}
        if (results.load(0) == 1 && results.load(1) == 2 && results.load(2) == 2 && results.load(3) == 1) {
            weakBehavior++;
        } else {
            nonWeakBehavior++;
        }
