...
__kernel void litmus_test(
  ...
  if (shuffled_ids[get_global_id(0)] == get_local_size(0) * 0 + 0) {
    ...
    uint r1 = atomic_load_explicit(&test_data[y], memory_order_relaxed);
    atomic_store_explicit(&results[1], r1, memory_order_seq_cst);
    atomic_store_explicit(&results[0], r0, memory_order_seq_cst);
  } else if (shuffled_ids[get_global_id(0)] == get_local_size(0) * 2 + 0) {
    if (stress_params[4]) {
      do_stress(scratchpad, scratch_locations, stress_params[5], stress_params[6]);
    }
    if (stress_params[0]) {
  ...
  } else if (shuffled_ids[get_global_id(0)] == get_local_size(0) * 3 + 0) {
    if (stress_params[4]) {
      do_stress(scratchpad, scratch_locations, stress_params[5], stress_params[6]);
    }
    if (stress_params[0]) {
    ...
    uint r2 = atomic_load_explicit(&test_data[y], memory_order_relaxed);
    uint r3 = atomic_load_explicit(&test_data[x], memory_order_relaxed);
    atomic_store_explicit(&results[2], r2, memory_order_seq_cst);
    atomic_store_explicit(&results[3], r3, memory_order_seq_cst);
