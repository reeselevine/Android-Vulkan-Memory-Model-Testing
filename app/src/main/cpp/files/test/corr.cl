...
static void spin(__global atomic_uint* barrier) {
  int i = 0;
  uint val = atomic_fetch_add_explicit(barrier, 1, memory_order_seq_cst);
  while (i < 1024 && val < 2) {
    val = atomic_load_explicit(barrier, memory_order_seq_cst);
    i++;
  }
}
...
__kernel void litmus_test(
  ...
  if (shuffled_ids[get_global_id(0)] == get_local_size(0) * 0 + 0) {
    ...
    atomic_store_explicit(&test_data[x], 1, memory_order_seq_cst);
    ...
    uint r0 = atomic_load_explicit(&test_data[x], memory_order_seq_cst);
    uint r1 = atomic_load_explicit(&test_data[y], memory_order_seq_cst);
